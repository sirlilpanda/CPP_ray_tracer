...
glm::vec3 SceneObject::getColor(float s, float t){
	return texture.getColorAt(s, t);
}
...
void SceneObject::setTexture(const char* filename){
	text_ = true;
	texture = TextureBMP(filename);
}
...
glm::vec2 SceneObject::getTextcoq(glm::vec3 hit){
	return glm::vec2(0, 0); 
};
